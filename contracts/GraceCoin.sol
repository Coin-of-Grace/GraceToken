...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "Coin of Grace";
    string internal constant SYMBOL = "GRACE";
    
    uint16 internal constant FEES_DIVISOR = 10**3;
    uint8 internal constant DECIMALS = 2;
    uint256 internal constant ZEROES = 18**DECIMALS;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 internal constant TOTAL_SUPPLY = 47000000 * ZEROES;
     ...
     */
    uint256 internal constant maxTransactionAmount = TOTAL_SUPPLY / 400; // 4% of the total supply
     ...
     * `swapAndLiquify` of `Liquifier` will be executed. Half of the tokens will be swapped for CELO 
     *  and together with the other half converted into a GRACE-CELO LP Token.
     * 
     * See: `Liquifier`
     */
    uint256 internal constant numberOfTokensToSwapToLiquidity = TOTAL_SUPPLY / 1400; // 0.14% of the total supply
    ...
    function _addFees() private {
        ...
        /// @inheritdoc	Copies all missing tags from the base function (must be followed by the contract name)(this));
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    function _isUnlimitedRecipient(address account) internal view returns(bool){
        ...
        // he/she wants
        return (account == owner() || account == foundationAddress);
    }
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    // UbeSwap 
    address private _mainnetRouterV2Address = 0x75F59534dd892c1f8a7B172D639FA854D529ada3;
    ...
    function _swapAndLiquify(uint256 amount) private lockTheSwap {
        ...
        // capture the contract's current CELO balance.
        // this is so that we can capture exactly the amount of CELO that the
        // swap creates, and not make the liquidity event include any CELO that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;
        
        // swap tokens for CELO
        _swapTokensForCELO(half); // <- this breaks the CELO -> HATE swap when swap+liquify is triggered

        // how much CELO did we just swap into?
    ...
    function _swapTokensForCELO(uint256 tokenAmount) private {
        
        // generate the uniswap pair path of token -> CELO
        ...
        // make the swap
        _router.swapExactTokensForSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 celoAmount) private {
        ...
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) = _router.addLiquidityETH{value: celoAmount}(
            ...
            0,
            // Bounds the extent to which the token/CELO price can go up before the transaction reverts.
            // 0 = accept any amount (slippage is inevitable)
            0,
    ...
    */
    ...
    /**
     * @dev The owner can withdraw CELO collected in the contract from `swapAndLiquify`
     *
     * 
     * The swapAndLiquify function converts half of the contractTokenBalance Coin of Grace tokens to CELO. 
     * For every swapAndLiquify function call, a small amount of CELO remains in the contract. 
     * This amount grows over time with the swapAndLiquify function being called 
     * throughout the life of the contract. The Grace contract does not contain a method 
     * to withdraw these funds, and the CELO will be locked in the Safemoon contract forever.
     * https://www.certik.org/projects/safemoon
     */
    function withdrawLockedCELO(address payable recipient) external onlyManager(){
        require(recipient != address(0), "Cannot withdraw the CELO balance to the zero address");
        require(withdrawableBalance > 0, "The CELO balance must be greater than 0");
...
abstract contract GraceToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    function _takeTransactionFees(uint256 amount, uint256 currentRate) internal override {
        ...
        for (uint256 index = 0; index < feesCount; index++ ){
            ...
            else if ( name == FeeType.ExternalToCELO){
                _takeFeeToCELO( amount, currentRate, value, recipient, index );
            }
            else {
    ...
    /**
     * @dev When implemented this will convert the fee amount of tokens into CELO/UBE
     ...
     */
    function _takeFeeToCELO(uint256 amount, uint256 currentRate, uint256 fee, address recipient, uint256 index) private {
...
contract GraceCoin is GraceToken{

    constructor() GraceToken(Env.Testnet){
 ...
 * - CELO/Ube collected/stuck in the contract can be withdrawn (see)
 */